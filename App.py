...
st.title("⚽ AI Football Bot Pro — Расширенный анализатор лиг и рисков")
...
with tab1:
    st.markdown("### 🚀 Поиск матчей по всем лигам и расчет рисков")
    st.write("Бот опрашивает расширенный список топ-лиг и вторых дивизионов, оценивая шансы и риски.")
    ...
    if st.button("🔎 Запустить сканирование матчей"):
        if not odds_api_key:
            ...
            with st.spinner("Анализ расширенного пула лиг и расчет рисков..."):
                ...
                st.success(f"Проверено матчей: {checked}. Добавлено новых ставок: {placed}.")
...
with tab3:
    ...
    st.write("Загрузите архив матчей, после чего запустите чистое обучение. Старые веса полностью сбросятся.")
...
with tab4:
    ...
    st.write("Бот рассчитывает вероятности исходов по распределению Пуассона, показывает детальный шанс на победу и риск проигрыша по расширенному списку европейских лиг.")
