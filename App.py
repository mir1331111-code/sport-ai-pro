...
from data.sources import parse_date
...
# ---------------- Auto-settle ----------------
def _auto_settle(D: dict) -> tuple[dict, int]:
    if usage.settle_remaining() <= 0:
        return D, 0
    from data.sources import tsdb_match_result
...
{clv['avg_clv']*100:+.2f}%</b> · N={clv['n']}</div></div>""",
    ...
    st.info("⚠️ Используйте проверенные кэфы. "
            "Paper-ставки не влияют на реальную статистику.")
