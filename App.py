...
# [РЕВЬЮ №1] дефолты пер-лига; дальше оптимизируются отдельно по каждой лиге
DEF_LP=lambda: {"w_shots":0.35,"rho":-0.13,"w_dc":0.72}

DIV_NAMES={"E0":"🏴󠁥󠁮󠁧󠁿 АПЛ","E1":"🏴󠁢󠁮󠁿 Чемпионшип","SC0":"🏴󠁢󠁣 Шотландия",
...
# [РЕВЬЮ №3] буфер ошибок вместо тихих except
ERR=[]
def log_err(tag,e):
    ERR.append(f"[{tag}] {type(e).__name__}: {e}")
    if len(ERR)>300: ERR.pop(0)

# ================= ДВИЖОК (пер-лига параметры) =================
class Engine:
    def __init__(self):
        self.elo={}
        ...
        self.lp=defaultdict(DEF_LP)          # [РЕВЬЮ №1] гиперпараметры по лигам
        self.hist=defaultdict(list)          # история по лигам для refit
        self.match_count=0
    ...
    def _probs_from(self,lh,la,rho,w,e,pde):
        p1,px=self._p1px(lh,la,rho)
        f1=w*p1+(1-w)*e*(1-pde);fd=w*px+(1-w)*pde;f2=max(1e-6,1-f1-fd)
        return f1,fd,f2
    def _fit_league(self,lg):  # [РЕВЬЮ №1] двухстадийный refit по лиге
        ...
        for gh,ga,sh,sa,e,pde,out in win:
            ...
            data.append(({r:self._p1px(lh,la,r) for r in (-0.20,-0.13,-0.06,0.0)},e,pde,out))
        best=None
    ...
    def predict(self,h,a,lg="G"):
        ...
        p1,px=self._p1px(lam_h,lam_a,P0["rho"])
        ...
        Mx=[[self._p(lam_h,i)*self._p(lam_a,j) for j in range(N)] for i in range(N)]
        btts=sum(Mx[i][j] for i in range(1,N) for j in range(1,N))
        ...
        return {"p1":f1,"x":fd,"p2":f2,"over":over,"btts":btts,"M":Mx,"agree":agree,
...
def clv_for(pick,odd,mkt,row):  # [РЕВЬЮ №2] CLV против Pinnacle (прокси closing)
...
# [РЕВЬЮ №4] редьюсеры состояния: чистые функции, возврат нового dict
def new_data():
...
def apply_settle(D,idx,outcome):
    ...
    if outcome=="push":
        ...
        pr=b["stake"]*(b["odds"]-1)
        b["status"]="won";D2["bank"]+=b["stake"]+pr
...
**📏 CLV** — перевес взятого кэфа над закрытием Pinnacle (положительный = модель бьёт рынок) ·
...
<div class="hero">
 ...
 <p>Пер-лига гиперпараметры · CLV-трекинг · лог ошибок · редьюсеры состояния · компонентный рендер</p>
 <div class="kpis">
                    ...
                    if odd:
                        ...
                        if item["ok"]:
                            ...
                            if best is None or ev>best[3]:
                                best=(mktk,pick,odd,ev,prob,stk)
                                card_clv=clv_for(pick,odd,mkt,r)
...
with tab3:
    ...
    if clvs:
        ...
        st.caption("📏 CLV появится после скана: он фиксируется в момент ставки против Pinnacle.")
    ...
    if st.button("▶️ Прогнать",type="primary"):
        ...
        if not log: st.warning("Нет сигналов: снизь edge или смени цель.")
            ...
            st.markdown(f"**📏 Средний CLV: {avg_clv*100:+.2f}%** "
                        +("✅ модель системно бьёт закрытие Pinnacle — плюс не случайный" if avg_clv>0.01
