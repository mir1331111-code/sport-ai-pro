...
API_TO_DIV = {
    "API_39": "E0", "API_40": "E1",
    "API_140": "SP1", "API_141": "SP2",
    "API_135": "I1", "API_136": "I2",
    "API_78": "D1", "API_79": "D2",
    "API_61": "F1", "API_62": "F2",
    "API_88": "N1", "API_144": "B1",
    "API_94": "P1", "API_203": "T1",
    "API_235": "R1", "API_197": "G1",
...
DIV_NAMES = {
    ...
    "G1": "🇬🇷 Греция", "R1": "🇷🇺 РПЛ",
    "C1": "🏆 Лига Чемпионов", "EL": "🏆 Лига Европы", "EC": "🏆 Лига Конференций",
}
...
def db_insert_bet(bet):
    with _db() as c:
        ...
        """, (
            ...
            bet.get("market"), bet.get("pick"), float(bet.get("odds") or 0),
            bet.get("closing_odds"), float(bet.get("stake") or 0),
            bet.get("prob"), bet.get("status", "pending"), bet.get("score"),
            bet.get("strat"), bet.get("odds_source"), bet.get("mode"),
            bet.get("fixture_id"), bet.get("date"), bet.get("date_iso"),
            bet.get("date_time"), bet.get("ev"), bet.get("clv"), bet.get("settled_at"),
        ))
...
def db_migrate_from_gist(D):
    existing = db_fetch_bets(limit=1)
    if existing:
        return False
    ...
    for b in bets:
        ...
        try:
            ...
            log_err("db_migrate_bet", e)
    if gd.get("bank") is not None:
        db_log_bank(float(gd["bank"]), event="migration")
    db_set_meta("migrated_from_gist_at", datetime.now().isoformat())
    return True


def compute_clv(placed_odds, closing_odds):
    if not placed_odds or not closing_odds:
        return None
    if placed_odds <= 1 or closing_odds <= 1:
        return None
    return (placed_odds / closing_odds) - 1


def db_update_clv_for_fixture(fixture_id, closing_odds_map):
    if not fixture_id or not closing_odds_map:
        return 0
    with _db() as c:
        c.row_factory = sqlite3.Row
        rows = c.execute(
            "SELECT id, pick, odds FROM bets WHERE fixture_id=? AND closing_odds IS NULL",
            (int(fixture_id),)).fetchall()
    updated = 0
    for r in rows:
        pick = r["pick"]
        closing = closing_odds_map.get(pick)
        if not closing:
            continue
        clv = compute_clv(float(r["odds"]), float(closing))
        db_update_bet(r["id"], closing_odds=float(closing), clv=clv)
        updated += 1
    return updated
...
def drawdown_stats(initial_bank=10000.0):
    ...
    if len(banks) < 2:
        return {"peak": initial_bank, "max_dd": 0.0, "max_dd_pct": 0.0,
                "current_dd": 0.0, "current_dd_pct": 0.0, "n_points": len(banks)}
    peak = banks[0]
    max_dd = 0.0
    max_dd_pct = 0.0
    for b in banks:
        ...
        if dd > max_dd:
            ...
            max_dd_pct = dd_pct
    current = banks[-1]
    current_peak = max(banks)
    current_dd = current_peak - current
...
def sync_bets_to_sqlite(D):
    ...
    try:
        ...
        for b in D.get("bets", []):
            ...
            with _db() as c:
                cur = c.execute(
                    "SELECT id FROM bets WHERE match=? AND pick=? AND date_iso=? LIMIT 1",
...
def _engine_sign(blob_bytes):
    if not ENGINE_HMAC_KEY:
        return ""
    return hmac.new(ENGINE_HMAC_KEY.encode(), blob_bytes, hashlib.sha256).hexdigest()
...
def api_request(api_key, endpoint, params=None, timeout=15, count_usage=True):
    ...
    try:
        r = _sess.get(url, headers=headers, params=params or {},
...
def api_fixtures_by_league(api_key, d_from, d_to, progress_cb=None):
    ...
    out = []
    rep = []
    leagues = [(39, "АПЛ", "E0"), (140, "Ла Лига", "SP1"), (135, "Серия A", "I1"),
               (78, "Бундеслига", "D1"), (61, "Лига 1", "F1"), (2, "ЛЧ", "C1"),
               (3, "ЛЕ", "EL"), (848, "ЛК", "EC"), (235, "РПЛ", "R1"), (203, "Суперлига", "T1")]
    total = len(leagues)
...
def api_fixture_result(api_key, fixture_id, count_usage=True):
    ...
    if cached is not None:
        return cached
    data, err = api_request(api_key, "fixtures", {"id": fixture_id},
                            timeout=10, count_usage=count_usage)
    if err:
        return None
...
def api_fixture_odds(api_key, fixture_id):
    ...
    for k, v in acc.items():
        ...
        if len(v) >= 5:
            vs = sorted(v)
            lo = max(1, int(len(vs) * 0.10))
            hi = len(vs) - lo
            v_trim = vs[lo:hi] if hi > lo else vs
            out[k] = sum(v_trim) / len(v_trim)
        else:
            out[k] = max(v)
...
def refine_with_real_odds(verdict, rows, real_odds, bank, kelly_frac):
    ...
    if verdict.get("is_action"):
        ...
        if stake > 0:
            best = (_market_type(pick), pick, real_odd, ev, prob, stake)
    return verdict, best
...
def tsdb_day(dstr):
    ...
    if cached is not None:
        return cached if isinstance(cached, list) else []
    try:
        r = _sess.get(f"https://www.thesportsdb.com/api/v1/json/3/eventsday.php",
...
def load_seasonal(div, season):
    ...
    try:
        ...
        if r.status_code != 200 or not r.content:
            log_err("load_seasonal", f"HTTP {r.status_code} {url}")
            return []
...
def build_verdict(P, thr, bank, kelly_frac, h_name, a_name, fh, fa, h2h_n):
    ...
    if top["prob"] >= thr:
        ...
        best = (_market_type(top["pick"]), top["pick"], top["odd"],
...
def migrate(D):
    ...
    if not isinstance(D.get("stats"), dict):
        D["stats"] = base["stats"]
    for s in ("won", "lost", "profit", "push", "void"):
        D["stats"].setdefault(s, 0)

    for b in D.get("bets", []):
        if isinstance(b, dict) and b.get("market") == "1X2":
            pick = b.get("pick", "")
            if pick not in ("П1", "X", "П2"):
                b["market"] = _market_type(pick)
    for b in D.get("bets", []):
        if isinstance(b, dict) and b.get("status") == "push":
            score = b.get("score") or ""
            if "void" in str(score).lower():
                b["status"] = "void"
...
def save_data(D):
    if CLOUD_IS_CLOUD:
        D_safe = clone(D)
        D_safe.get("meta", {}).pop("api_key", None)
        return gist_save_json(CLOUD_GIST_ID, HISTORY_FILE, D_safe)
    return True
...
def engine_cache_fp(season, extra, matrix_n):
    return (APP_VERSION, season, int(matrix_n), str(extra))
...
def api_find_fixture_id(api_key, home, away, date_iso, count_usage=True):
    ...
    if cached is not None:
        ...
        data, err = api_request(api_key, "fixtures", {"date": date_iso},
                                timeout=20, count_usage=count_usage)
        if err:
            return None
...
def auto_settle(D):
    ...
    for idx, b in enumerate(D2["bets"][:]):
        ...
        if not fid and bd and bd <= now - timedelta(hours=2) \
           and " vs " in (b.get("match") or "") \
    ...
    for idx, b in enumerate(D2["bets"][:]):
        ...
        if bd and now - bd > timedelta(hours=48):
            D2 = apply_settle(D2, idx, "void", score="void (no result)")
            changed += 1
...
def stadium_bg(div):
    return STADIUM_WALLS.get(norm_div(div), STADIUM_WALLS["DEFAULT"])
...
<div class="vcard" style="background:{bg_grad};border:1px solid rgba(255,255,255,.10);border-radius:20px;padding:0;margin-bottom:14px;overflow:hidden;">
  ...
  <div style="background:{main_bg};border-top:1px solid {main_border};border-bottom:1px solid {main_border};padding:14px 20px;">
    <div style="font-size:1.15rem;font-weight:900;color:#fff;margin-bottom:8px;">{main_title}</div>
...
<div style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.10);border-radius:12px;padding:10px 14px;margin-bottom:10px;">
<div style="color:#7dd3fc;font-size:.7rem;text-transform:uppercase;font-weight:700;">📈 CLV</div>
<div style="font-size:.9rem;color:#e6eaf2;">Avg: <b style="color:{'#34d399' if clv['avg_clv']>0 else '#f87171'}">{clv['avg_clv']*100:+.2f}%</b> · N={clv['n']} · +{clv['positive_share']*100:.0f}%</div>
</div>
<div style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.10);border-radius:12px;padding:10px 14px;margin-bottom:10px;">
...
with tab2:
    ...
    if not D["bets"]:
        ...
        if st.button("📥 Экспорт CSV"):
            buf = io.StringIO()
            w = csv.writer(buf)
            w.writerow(["Match", "League", "Pick", "Market", "Odds", "Stake", "Prob", "Status", "Score", "Date"])
            for b in D["bets"]:
                if not isinstance(b, dict):
                    continue
                match_display = b.get("match_ru") or translate_match(b.get("match", ""))
                w.writerow([match_display, b.get("league", ""), b.get("pick", ""),
                            b.get("market", ""), b.get("odds", ""), b.get("stake", ""),
                            b.get("prob", ""), b.get("status", ""), b.get("score", ""),
                            b.get("date", "")])
...
<div class="betcard {st_}" style="padding:14px 18px;">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;">
    <div>
      ...
      <div style="color:#8b93a7;font-size:.78rem;margin-top:4px;">{esc(str(b.get('league', '—')))} · {esc(str(b.get('date', '—')))} · рынок {esc(str(market))}</div>
    </div>
...
with tab3:
    ...
    if SQLITE_BOOT_OK and use_sqlite_enabled(D):
        ...
        if sqlite_bets:
            ...
            n_lost = sum(1 for b in sqlite_bets if b.get("status") == "lost")
            n_push = sum(1 for b in sqlite_bets if b.get("status") == "push")
...
with tab5:
    ...
    st.caption("Источник: football-data.co.uk. EV считается против реальной цены.")
