...
"""
  ...
  if st.session_state.history:
    ...
    if not matches_data:
      st.warning("В текущей записи нет матчей. Нажмите «🚀 Сканировать Линию».")
    else:
        ...
        with cols[col_idx]:
          with st.container(border=True):
            ...
            if card.get("score_changed"):
              st.markdown(
                  "<div class='score-badge-live'>🔥 СЧЕТ ИЗМЕНИЛСЯ:"
            ...
            if card.get("user_loss_reason"):
              st.markdown(
                  "<div class='loss-reason-box'>🚨 **Причина минуса:**"
            ...
            with b_c2:
              with st.popover("🔴 Минус"):
                st.write("🧠 **Обучение ИИ: почему ставка не зашла?**")
                ...
                if st.button("Сохранить и обучить ИИ", key=f"pop_btn_{idx}"):
                  card["status"] = "❌ Проигрыш"
                  card["user_loss_reason"] = (
                      custom_r if custom_r else reason_opt
                  )
                  save_history(st.session_state.history)
                  st.success("Причина сохранена в память ИИ!")
                  st.rerun()
        ...
        with cols[col_idx]:
          with st.container(border=True):
            st.markdown(
                ...
                f" `{card.get('score')}` [{card.get('game_phase', 'до перерыва')}]"
            )
            st.markdown(
            ...
            if card.get("user_loss_reason"):
              st.caption(f"🚨 Причина: {card.get('user_loss_reason')}")
