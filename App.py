"""NEURO BET PRO v12.9.2 — security + hold-out + caching."""
...
DIV_NAMES = {"E0":"🏴󠁧󠁢󠁥󠁮󠁧󠁿 АПЛ","E1":"🏴󠁧󠁢󠁥󠁮󠁿 Чемпионшип","D1":"🇩🇪 Бундеслига",
...
if CLOUD_IS_CLOUD and not ENGINE_HMAC_KEY:
    _HMAC_WARNING = ("⚠️ ENGINE_HMAC_KEY не задан в secrets! Engine не будет загружаться из Gist. "
                     "Добавь в .streamlit/secrets.toml: ENGINE_HMAC_KEY='secret'")
ERR=[]
...
def db_init():
    ...
    try:
        ...
        SQLITE_BOOT_OK=True; SQLITE_BOOT_ERROR=""
        return True
...
def engine_save_gist(fp,engine):
    ...
    if not ENGINE_HMAC_KEY:
        log_err("engine_save_gist","ENGINE_HMAC_KEY not set — refusing unsigned pickle")
        return False
    try:
        ...
        return _gist_save_raw(CLOUD_GIST_ID,ENGINE_GIST_FILE,sig+"\n"+blob)
    except Exception as e:
        log_err("engine_save_gist",e); return False
def engine_load_gist(fp):
    if not CLOUD_IS_CLOUD: return None
    if not ENGINE_HMAC_KEY:
        log_err("engine_load_gist","ENGINE_HMAC_KEY not set — rejecting unsigned pickle")
        return None
    try:
        payload=_gist_load_raw(CLOUD_GIST_ID,ENGINE_GIST_FILE)
        if not payload: return None
        if "\n" not in payload:
            log_err("engine_load_gist","no signature — rejecting"); return None
...
class Engine:
    ...
    def learn_step(self,h,a,hg,ag,row=None,lg="G",match_num=None,total=None,match_date=None):
        ...
        self.match_count+=1
        if self.match_count%150==0 and not is_holdout: self.refit_calibrator()
...
def save_data(D):
    if CLOUD_IS_CLOUD:
        Ds=clone(D); Ds.get("meta",{}).pop("api_key",None)
...
def auto_settle(D):
    ...
    if not api_key: return D,0
    if settle_usage_remaining()<=0: return D,0
    ...
    for idx,b in enumerate(D2["bets"][:]):
        ...
        if not fid: continue
        if settle_usage_remaining()<=0: break
        if bd and bd>now: continue
        ck=f"api_result_{fid}"
        cached=disk_cache_get(ck,3600)
        if cached: res=cached
        else:
            res=api_fixture_result(api_key,fid,count_usage=False); settle_usage_increment(1)
        if not res: continue
...
def render_verdict_card(c,thr):
    ...
    if is_action and not has_real:
        warn=("<div style='color:#fde68a;font-size:.78rem;margin-top:8px;'>⚠️ Реального кэфа не нашли "
              "(лимит API или рынок не покрыт) — ставка не открыта, это информсигнал.</div>")
    return f"""
<div class="vcard" style="background:{bg};border:1px solid rgba(255,255,255,.10);border-radius:20px;padding:0;margin-bottom:14px;overflow:hidden;">
  ...
  <div style="padding:14px 20px;">
    ...
    <div style="color:#7dd3fc;font-size:.72rem;text-transform:uppercase;font-weight:700;margin-bottom:6px;letter-spacing:1px;">Альтернативы</div>{alt_html}</div></div>"""
...
if _HMAC_WARNING: st.warning(_HMAC_WARNING,icon="🔐")
...
<div style="color:#8b93a7;font-size:.75rem;">авто-проверок сегодня</div></div>""",unsafe_allow_html=True)
    if CLOUD_IS_CLOUD:
        hs_="🟢 Установлен" if ENGINE_HMAC_KEY else "🔴 НЕ задан"
        st.markdown(f"""
<div style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.10);border-radius:12px;padding:10px 14px;margin-bottom:10px;">
<div style="color:#7dd3fc;font-size:.7rem;text-transform:uppercase;font-weight:700;">🔐 HMAC</div>
<div style="font-size:.9rem;color:#e6eaf2;">{hs_}</div></div>""",unsafe_allow_html=True)
...
<div style="font-size:.85rem;color:#e6eaf2;">Ставок: <b>{s['bets']}</b> · Pending: <b>{s['pending']}</b> · {s['size_kb']:.1f} KB</div></div>
<div style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.10);border-radius:12px;padding:10px 14px;margin-bottom:10px;">
<div style="color:#7dd3fc;font-size:.7rem;text-transform:uppercase;font-weight:700;">📈 CLV</div>
<div style="font-size:.9rem;color:#e6eaf2;">Avg: <b style="color:{'#34d399' if clv['avg_clv']>0 else '#f87171'}">{clv['avg_clv']*100:+.2f}%</b> · N={clv['n']}</div></div>
<div style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.10);border-radius:12px;padding:10px 14px;margin-bottom:10px;">
<div style="color:#7dd3fc;font-size:.7rem;text-transform:uppercase;font-weight:700;">📉 Drawdown</div>
<div style="font-size:.85rem;color:#e6eaf2;">Max: <b style="color:#f87171">-{dd['max_dd_pct']*100:.1f}%</b> · Сейчас: <b>{dd['current_dd_pct']*100:.1f}%</b> · Sharpe: <b>{shp:.2f}</b></div></div>""",unsafe_allow_html=True)
    if st.button("♻️ Сбросить счётчики"):
        api_usage_reset(); settle_usage_reset(); st.toast("Счётчики сброшены",icon="♻️"); st.rerun()
...
<div class="nbr-bar"><div class="nbr-bar-fill" style="width:{pct*100:.0f}%"></div></div></div></div>""",unsafe_allow_html=True)
                ...
                if engine is None:
                    ...
                    logs.append(f"📊 Hold-out: {min(CALIB_HOLDOUT,total_matches)} матчей не использовались")
                ...
                for r in src_rows:
                    ...
                    if verdict.get("is_action"):
                        ...
                        if fid_ and api_usage_remaining()>20 and matches_with_best<15:
                            ...
                            verdict["real_odds"]=False; verdict["odd"]=None; verdict["ev"]=None; best=None
                ...
                for c in cards:
                    ...
                    new_bets.append({"match":c["match"],"match_ru":c["match_ru"],"div":c["div"],
                        ...
                        "prob":prob,"status":"pending","strat":"value","odds_source":"API-Football",
...
<div class="betcard {st_}" style="padding:14px 18px;">
 <div style="display:flex;justify-content:space-between;align-items:flex-start;">
  <div><div style="font-size:1rem;font-weight:800;color:#fff;">{icon} {esc(md_)}{score}</div>
   <div style="color:#8b93a7;font-size:.78rem;margin-top:4px;">{esc(str(b.get('league','—')))} · {esc(str(b.get('date','—')))} · рынок {esc(str(b.get('market','—')))}</div></div>
  <div style="text-align:right;"><div style="color:#fbbf24;font-size:1.1rem;font-weight:800;">{esc(str(b.get('pick','—')))}</div>
   <div style="color:#34d399;font-weight:700;">P {prob*100:.0f}%</div></div></div>
