...
st.title("⚽ AI Football Bot Pro — Расширенный анализатор лиг")
...
current_bank = st.session_name = st.session_state.app_data["bank"]
...
LEAGUES = [
    ...
    "soccer_efl_champ",  # Англия Чемпионшип
...
tab1, tab2, tab3, tab4 = st.tabs(
    ["🎯 Анализ и Риски", "📊 Статистика", "🧠 Обучение с нуля", "⚙️ О системе"]
)

with tab1:
  st.markdown("### 🚀 Поиск матчей по всем лигам и расчет рисков")
  st.write(
      "Теперь бот опрашивает расширенный список топ-лиг и вторых дивизионов."
  )
      ...
      with cols[1]:
        if status == "pending":
          ...
          if st.button("↩️ Сбросить", key=f"reset_{idx}"):
            if status == "won":
              st.session_state.app_data["bank"] -= (
                  stake_val * odd_val - stake_val
              )
            else:
...
with tab3:
  ...
  st.write(
      "Загрузите архив и запустите чистое обучение. Старые веса обнулятся."
  )
...
with tab4:
  ...
  st.write(
      ...
      " показывает четкий шанс на победу и риск проигрыша по расширенному"
      " списку лиг."
  )
