...
.form5{font-size:.72rem;letter-spacing:1px;margin-bottom:4px;color:#cbd5e1}
.form5 b{padding:1px 4px;border-radius:4px;margin-right:2px}
...
class Engine:
  ...
  def form_str(self, t):
    ...
    for x in self.st[t]["form"][-5:]:
      ix = int(x)
      cls_ = "w" if ix == 3 else ("d" if ix == 1 else "l")
      txt = "В" if ix == 3 else ("Н" if ix == 1 else "П")
      res.append(f'<b class="{cls_}">{txt}</b>')
    return "".join(res) or "—"
...
if mode == "🎯 Прогнозы на матчи":
  ...
  with tab_matches:
    ...
    for row in fixtures[:50]:
      ...
      count += 1
      form_h = engine.form_str(h)
      form_a = engine.form_str(a)
      league_label = DIV_NAMES.get(div, div)
          ...
          f"""
            <div class="mcard value">
                <div class="mhead">
                    ...
                    <span class="chip">{match_date}</span>
                    <span class="badge val">ВЫГОДА: {pick} ({max_p*100:.1f}%)</span>
                </div>
                <div class="teams">{h} <span>vs</span> {a}</div>
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; font-size:0.8rem; margin-top:6px;">
                    <div>Форма хозяев: <div class="form5">{form_h}</div></div>
                    <div>Форма гостей: <div class="form5">{form_a}</div></div>
                </div>
                <div class="verdict">
                    <b>Прогноз движка:</b> Ожидаемые голы ($\lambda$): <b>{P['lams'][0]:.2f}</b> — <b>{P['lams'][1]:.2f}</b><br>
                    Вероятности: П1: <b>{P['p1']*100:.1f}%</b> | Х: <b>{P['x']*100:.1f}%</b> | П2: <b>{P['p2']*100:.1f}%</b>
                    {f"<br>Рекомендация: Ставка <b>{pick}</b> по кф. <b>{odd}</b> | Келли: <b>{kelly(max_p, odd, 10000.0, 0.25):.1f} руб.</b>" if odd else ""}
                </div>
            </div>
            """,
          unsafe_allow_html=True,
      ...
      with st.expander(
          f"📊 Матрица вероятностей счетов ({h} — {a})", expanded=False
      ):
