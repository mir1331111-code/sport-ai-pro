...
def insert_bets_batch(bets: Iterable[dict]) -> int:
    rows = []
    for b in bets:
...
def fetch_bets(status: Optional[str] = None, limit: int = 100000) -> list[dict]:
    q = "SELECT * FROM bets"
    params: list = []
    if status:
        q += " WHERE status=?"
...
@_cache(ttl=30, show_spinner=False)
def bank_history(limit: int = 5000) -> list[dict]:
    with _db() as c:
        c.row_factory = sqlite3.Row
        rows = c.execute(
...
def invalidate_caches() -> None:
    """Вызывать после любой мутации bets/bank."""
    fetch_bets.clear()
    clv_summary.clear()
    bank_history.clear()
