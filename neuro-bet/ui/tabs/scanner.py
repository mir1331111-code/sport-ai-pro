...
def _train_engine(matrix_n: int, logs: list, update_loader) -> Engine:
    """Обучает Engine с нуля, кладёт в кэш."""
    ...
    for i, dv in enumerate(train_divs):
        ...
        update_loader(f"История [{i+1}/{len(train_divs)}] — "
                      f"{DIV_NAMES.get(dv, dv)}", pct, logs)
...
def _load_or_train_engine(matrix_n: int, logs: list, update_loader) -> Engine:
    eng_key = f"neuro_engine_v{APP_VERSION}_{matrix_n}"
    cached = cache_get(eng_key, CACHE_TTL["engine"])
    if cached and cached.get("fp") == APP_VERSION:
        engine_data = cached.get("engine")
        if engine_data:
            logs.append("💾 Engine из кэша")
            return Engine._from_dict(engine_data) if hasattr(Engine, "_from_dict") else None
    engine = _train_engine(matrix_n, logs, update_loader)
    try:
        cache_put(eng_key, {"fp": APP_VERSION, "engine": engine})
    except Exception:
        # Engine не JSON-сериализуем → сохраняем через pickle ТОЛЬКО локально
...
def _load_engine_from_disk(matrix_n: int):
    """Пытается загрузить Engine из локального pickle-файла (только локально)."""
    import pickle, gzip, os
    from config import DISK_CACHE_DIR
    p = os.path.join(DISK_CACHE_DIR, f"engine_{matrix_n}.pkl.gz")
...
def render(min_prob: float, kelly_frac: float, matrix_n: int) -> None:
...
<div class="nbr-text"><b>{text}</b>
<div class="nbr-bar"><div class="nbr-bar-fill"
 style="width:{pct*100:.0f}%"></div></div></div></div>""", unsafe_allow_html=True)
                if logs:
                    log_ph.markdown(
            ...
            # Engine
            engine = _load_engine_from_disk(matrix_n)
            if engine is None:
                engine = _load_or_train_engine(matrix_n, logs, update_loader)
            else:
                logs.append(f"💾 Engine из диска (обучено "
                            f"{getattr(engine, 'trained_n', '?')})")
            if engine is None:
                st.error("Не удалось обучить движок.")
                return
            ...
            for r in src_rows:
                ...
                if verdict.get("is_action"):
                    ...
                    if real_odds:
                        ...
                        odds_source = "market"
                    if best is None:
                        # fallback: estimated кэф, минимальная ставка
