...
def build_alternatives(rows: list[dict], main_pick: str) -> list[dict]:
    alt = []
...
def build_verdict(P: dict, thr: float, bank: float, kelly_frac: float,
                  ...
                  h2h_n: int) -> tuple[dict, list[dict], None]:
    """best всегда None — решение о ставке принимает вызывающий код
    после получения реальных кэфов. fair_odd используется как fallback."""
    probs = {
...
def refine_with_real_odds(verdict: dict, rows: list[dict],
                          real_odds: dict | None,
                          bank: float, kelly_frac: float):
    """Обновляет rows и verdict реальными кэфами. Возвращает (verdict, best)."""
    for r in rows:
